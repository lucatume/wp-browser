		...
		foreach ( $this->get_keys( $table_name ) as $key ) {
			...
			$cols = array_map(
				function ( $column ) use ( $table_name, $key_length_limit ) {
					...
					if ( substr_compare($data_type, 'char', -strlen('char')) === 0 ||
						substr_compare($data_type, 'text', -strlen('text')) === 0 ||
						substr_compare($data_type, 'blob', -strlen('blob')) === 0 ||
						strncmp($data_type, 'var', strlen('var')) === 0
					) {
