...
class WP_SQLite_Information_Schema_Reconstructor {
	...
	private function generate_key_definition( string $table_name, array $key_info, array $column_types ): string {
		 ...
		 */
		if ( strncmp($name, "{$table_name}__", strlen("{$table_name}__")) === 0 ) {
		 ...
		 */
		if ( strncmp($name, 'sqlite_autoindex_', strlen('sqlite_autoindex_')) !== 0 ) {
		...
		foreach ( $key_columns as $column ) {
			 ...
			 */
			if (
				substr_compare($column_type, 'char', -strlen('char')) === 0
				|| substr_compare($column_type, 'text', -strlen('text')) === 0
				|| substr_compare($column_type, 'blob', -strlen('blob')) === 0
				|| strncmp($column_type, 'var', strlen('var')) === 0
			) {
				$cols[] = sprintf(
					'%s(%d)',
	...
	private function generate_column_default( string $mysql_type, ?string $default_value ): ?string {
		...
		if ( strncmp($uppercase_default_value, "X'", strlen("X'")) === 0 ) {
	...
	private function get_cached_mysql_data_type( string $table_name, string $column_or_index_name ): ?string {
		...
		if ( false !== $open_par_index ) {
			...
			$looks_like_wc_table = strpos($table_name, 'wc_') !== false || strpos($table_name, 'woocommerce_') !== false;
	...
	private function get_mysql_column_type( string $column_type ): string {
		...
		if ( strpos($type, 'TEXT') !== false || strpos($type, 'CHAR') !== false || strpos($type, 'CLOB') !== false ) {
			return 'text';
		}
		...
		if ( strpos($type, 'REAL') !== false || strpos($type, 'FLOA') !== false ) {
			return 'float';
		}
		if ( strpos($type, 'DOUB') !== false ) {
			return 'double';
		}
