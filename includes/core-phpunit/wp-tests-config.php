...
if ( ! isset( $wpLoaderConfig ) ) {
	if ( ! ( $wpInstalling ) ) {
		throw new RuntimeException(
			'This file should be included from either the WPLoader Module or the ' .
	...
	if ( $decodedWpLoaderEnvVar === false ) {
		throw new RuntimeException(
			'The WPLOADER_CONFIG environment variable is empty or not a valid base64 ' .
			'encoded string.'
		);
	}

	$wpLoaderConfig = unserialize( $decodedWpLoaderEnvVar, array( 'allowed_classes' => false ) );

	if ( $wpLoaderConfig === false || ! is_array( $wpLoaderConfig ) ) {
		throw new RuntimeException(
			'The WPLOADER_CONFIG environment variable is not a valid' .
			' serialized array.'
		);
	}
...
if ( ! empty( $wpLoaderIncludeWpSettings ) ) {
	...
	$_wp_menu_nopriv = $_wp_submenu_nopriv = array();
