...
Since the `WPLoader::loadOnly` parameter is not set or set to `false` both the WPLoader module and the WPDb one are trying to populate the database.
If you want to fill the database with a dump then keep using the WPDb module but set the `WPLoader::loadOnly` parameter to `true` and make sure that, in the suite configuration file, in the `modules` section, the WPDb module comes before the WPLoader one.

If you are, instead, trying to run integration tests you do not probably need the WPDb module or should set the `populate` and `cleanup` arguments to `false`
