...
class utilsTest extends \Codeception\Test\Unit
{
    ...
    {
        $map = dsnToArr('mysql:host=foo;dbname=bar;port=2345');
        ...
        $this->assertEquals('port', $map('2345','3306'));
    }

    public function test_dsnToArr_with_string_missing_prefix()
    {
        $map = dsnToArr('host=foo;dbname=bar;port=2345');
    ...
    {
        $map = dsnToArr('sqlite:/foo/bar.sqlite');
    ...
    {
        $map = dsnToArr('sqlite2::memory::');

        $this->assertEquals('sqlite2', $map['type']);
        $this->assertEquals('sqlite2', $map('prefix'));
        $this->assertEquals('localhost', $map('host','localhost'));
        $this->assertEquals(true, $map('memory','/db.sqlite'));
    }
}
