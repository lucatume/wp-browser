...
class WPLoaderThemeBackupTest extends \Codeception\Test\Unit
{
    ...
    {
        $wpRootDir = $this->phase('FS::tmpDir', function () {
            return FS::tmpDir('wploader_');
        });
        ...
        $db = $this->phase('MysqlDatabase::create', function () use ($dbName, $dbUser, $dbPassword, $dbHost) {
            return (new MysqlDatabase($dbName, $dbUser, $dbPassword, $dbHost))->create();
        });
        $scaffolded = $this->phase('Installation::scaffold', function () use ($wpRootDir) {
            return $this->fastScaffold($wpRootDir, 'latest');
        });
        $configured = $this->phase('configure (mysql)', function () use ($scaffolded, $db) {
            return $scaffolded->configure($db);
        });
        $installation = $this->phase('install (mysql)', function () use ($configured) {
        ...
        $this->phase('wp-cli scaffold child-theme', function () use ($installation) {
            return $installation->runWpCliCommandOrThrow([
                'scaffold',
                'child-theme',
                'some-child-theme',
        ...
        $installationOutput = $this->phase(
            ...
            function () use ($isolationClosure) {
                return $this->assertInIsolation($isolationClosure);
            }
        );
    }
