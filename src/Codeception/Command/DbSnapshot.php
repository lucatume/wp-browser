...
class DbSnapshot extends Command
{
    ...
    protected function configure()
    {
        $this->setName('db:snapshot')
            ...
            ->addOption('pass', 'p', InputOption::VALUE_OPTIONAL, 'If set the specified password will be used to connect to the database', 'root')
            ->addOption('dump-file', null, InputOption::VALUE_OPTIONAL, 'If set the local version of the database will be dumped in the specified file; should be absolute path or a path relative to the root folder.')
            ->addOption('dist-dump-file', null, InputOption::VALUE_OPTIONAL, 'If set the distribution version of the database will be dumped in the specified file; should be absolute path or a path relative to the root folder.')
            ...
            ->addOption('local-domain', null, InputOption::VALUE_OPTIONAL, 'The local setup domain that should be replaced in the distribution version of the dump file.', 'http://local.dev')
            ->addOption('dist-domain', null, InputOption::VALUE_OPTIONAL, 'The distribution setup domain that should be used in the distribution version of the dump file.', 'http://dist.dev');
    }

    public function __construct($name = null, MySQLDumpFactoryInterface $pdoFactory = null)
    {
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!file_put_contents($dumpFile, $dumpContents)) {
        ...
        $localDomain = $input->getOption('local-domain');
        $distDomain = $input->getOption('dist-domain');
        $distDumpContents = str_replace($localDomain, $distDomain, $dumpContents);

        if (!file_put_contents($distDumpFile, $distDumpContents)) {
