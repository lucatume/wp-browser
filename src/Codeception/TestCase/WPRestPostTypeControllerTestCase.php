...
abstract class WPRestPostTypeControllerTestCase extends WPRestControllerTestCase
{
    protected function check_post_data($post, $data, $context, $links)
    {
        ...
        if ($links) {
            ...
            $this->assertEquals($links['self'][0]['href'],
            ...
            foreach ($taxonomies as $key => $taxonomy) {
                ...
                $this->assertEquals(add_query_arg('post', $data['id'], rest_url('wp/v2/'.$taxonomy->rest_base)),
    ...
    {
        return wp_parse_args($args, $this->set_post_data([
            'title' => [
                'raw' => 'Post Title',
            ],
            'content' => [
