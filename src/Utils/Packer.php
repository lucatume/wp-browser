...
final class Packer
{
     ...
     */
    private $packReferences = [];

    /**
     * @var int
     */
    private $packRefCounter = 0;

    /**
     * @var array<string, mixed>
     */
    private $unpackReferences = [];

    public function __construct(bool $nullifyClosures = false)
    {
        $this->nullifyClosures = $nullifyClosures;
    }
    ...
    public function unpack(string $value)
    {
        ...
        try {
            $decoded = json_decode($value, true, 512, 0);
            if (!is_array($decoded)) {
     ...
     * @param mixed $value
     */
    private function packValue($value): array
    {
        $type = gettype($value);
        ...
        switch ($type) {
            ...
            default:
                throw new PackerException("Unsupported type: $type");
        }
    }
     ...
     * @param object $value
     */
    private function packObject($value, string $objHash): array
    {
     ...
     * @return mixed
     */
    private function unpackValue(array $data)
    {
        $type = $data['type'];
        $value = $data['value'];
        ...
        switch ($type) {
            case 'boolean':
                return $this->unpackBoolean($value);
            case 'string':
                return $this->unpackString($value);
            case 'integer':
                return $this->unpackInteger($value);
            case 'float':
                return $this->unpackFloat($value);
            case 'null':
                return $this->unpackNull();
            case 'resource':
                return $this->unpackResource();
            case 'array':
                return $this->unpackArray($value);
            case 'object':
                return $this->unpackObject($value);
            default:
                throw new PackerException("Unknown type: {$type}");
        }
    }
     ...
     * @param mixed $value
     */
    private function unpackBoolean($value): bool
    {
     ...
     * @param mixed $value
     */
    private function unpackString($value): string
    {
        if (!is_string($value)) {
     ...
     * @param mixed $value
     */
    private function unpackInteger($value): int
    {
        if (is_int($value)) {
            return $value;
        }
     ...
     * @param mixed $value
     */
    private function unpackFloat($value): float
    {
        if ($value === 'NAN') {
            return NAN;
        }
     ...
     */
    private function unpackArray($value): array
    {
        if (!is_array($value)) {
     ...
     * @return object
     */
    private function unpackObject($value)
    {
     ...
     * @return object
     */
    private function unpackIncompleteClass(string $className, array $value)
    {
     ...
     * @return mixed
     */
    private function unpackReference($value)
    {
     ...
     * @param mixed $value
     */
    private function isPackedValue($value): bool
    {
    ...
    private function doPackClosure(Closure $closure): array
    {
        ...
        $closureCalledClass = method_exists($reflection, 'getClosureCalledClass') ? $reflection->getClosureCalledClass() : $reflection->getClosureScopeClass();
    ...
    private function unpackClosure(array $value): Closure
    {
        ...
        if ($value['useContext'] !== null) {
            ...
            if (strpos($value['useContext'], '@closureReference') !== false
     ...
     */
    private static $packingClosures = [];

    /**
     * @var \stdClass|null
     */
    private static $bindSentinel;
    ...
    {
        self::$bindSentinel = self::$bindSentinel ?? new stdClass();
     ...
     */
    private static $sourceFileImportsCache = [];
