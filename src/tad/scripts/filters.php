...
$just_now = fn() => (object)['last_checked' => time(), 'version_checked' => $GLOBALS['wp_version']];
...
wpbrowser_addFilter('pre_site_transient_browser_' . md5('Symfony2 BrowserKit'), fn() => true, 10, 1);

// Block the dashboard feed fetching
wpbrowser_addFilter('init', function () {
    add_filter('pre_transient_dash_' . md5('dashboard_primary_' . get_locale()), fn() => 'Disabled');
});

// Block the compression test check
wpbrowser_addFilter('pre_option_can_compress_scripts', fn() => 1);
