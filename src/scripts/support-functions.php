 ...
 */
function cli_headers_list(): array
{
    $headers = [];
    global $cli_headers;
    foreach (\array_keys($cli_headers) as $value) {
        // Get the header name
        $parts = \explode(':', $value);
        if (\count($parts) > 1) {
            $name = \trim(\array_shift($parts));
            // Build the header hash map
            $headers[$name] = \trim(\implode(':', $parts));
        }
    }
...
function wpbrowser_addFilter($tag, $function_to_add, $priority = 10, $accepted_args = 1): bool
{
    global $wp_filter;
