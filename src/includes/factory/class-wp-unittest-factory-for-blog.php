...
class WP_UnitTest_Factory_For_Blog extends WP_UnitTest_Factory_For_Thing {
	...
	function __construct( $factory = null ) {
		...
		$this->default_generation_definitions = ['domain' => $current_site->domain, 'path' => new WP_UnitTest_Generator_Sequence( $base . 'testpath%s' ), 'title' => new WP_UnitTest_Generator_Sequence( 'Site %s' ), 'site_id' => $current_site->id];
	}

	function create_object( $args ) {
		global $wpdb;
		$meta = $args['meta'] ?? ['public' => 1];
